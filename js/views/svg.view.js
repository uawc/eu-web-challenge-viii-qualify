...
define(['jquery', 'underscore', 'backbone', 'templates', 'views/item.view', 'models/item.model'], function ($, _, Backbone, templates, ItemView, ItemModel) {
   ...
   * @param model - {object}
   * */
		_renderChild: function _renderChild(model) {
			if (!model.get('isHidden')) {
				this.$el.find('svg').append(new ItemView({ model: model }).el);
			}
		},
   ...
   * @private
   * */
		_onTextClick: function _onTextClick(e) {
			var currentModel = this.collection.at($(e.target).attr('id'));
   ...
   * */
		_collapseTreeElement: function _collapseTreeElement(parentElement) {
   ...
   * @param parentElement - {model}
   * */
		_expandTreeElement: function _expandTreeElement(parentElement) {
			parentElement.toggleIsClosed();
			this.collection.toggleNodes(parentElement, false);
		}
	});
});
//# sourceMappingURL=svg.view.js.map
